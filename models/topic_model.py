...
class Topic_Model:
    def __init__(self, base = 'LDA', k=3, use_AE = True, gamma = 1, embedding = None,
                 saved_embedding_dir = None, verbose= False, embedding_batch_size=None,
        ...
        if embedding is None and base is None:
            ...
            self.id = self.method + '_k_' + str(self.k) + '_' + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
        elif self.base is None:
            self.method =  "EMBEDDING"
            self.embedding = embedding
            self.id = self.method + '_k_' + str(self.k) + '_' + self.embedding + '_' + datetime.now().strftime(
                "%Y_%m_%d_%H_%M_%S")
        else:
            self.method = self.base + "_EMBEDDING"
            self.embedding = embedding
            self.id = self.method + '_k_' +  str(self.k) + '_' + self.embedding + '_' + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    ...
    def vectorize(self, sentences, token_lists, method=None):
        ...
        if method == 'LSI' or method == 'LDA':
            ...
            if self.base == 'LSI':
                self.basemodel = gensim.models.lsimodel.LsiModel(self.corpus, num_topics=self.k, id2word=self.dictionary)
            elif self.base == 'LDA':
                self.basemodel = gensim.models.ldamodel.LdaModel(self.corpus, num_topics=self.k, id2word=self.dictionary,
                                                passes=20)
            ...
            if self.verbose:
                ...
                print (vec)
                print ('\n According to {}, the top words for each topic should be, '.format(self.base))
            ...
            if self.saved_embedding_dir is None:
                sentence_embedding, attention , embedding_tokens = get_embeddings(self.embedding,
                                                    sentences=sentences, batch_size = self.embedding_batch_size,
                                                    output_attention = self.output_attention)
            else:
                # use previously generated embedding
                if self.saved_embedding_dir is not None:
                    sentence_embedding = get_embeddings(saved_embedding_dir=self.saved_embedding_dir)
                #use previously generated attention vector
                if self.saved_attention_dir is not None:
                    attention = get_embeddings(saved_attention_dir=self.saved_attention_dir)
                else:
    ...
    def fit_AE(self, vec, epoch):
        ...
        if self.use_AE:
            ...
            self.AE = Autoencoder(latent_dim=32, input_dim=input_size, epochs=epoch, batch_size=64)

            if self.verbose:
                print("Using the following autoencoder to learn the latent representation of the vector")
    ...
    def fit(self, sentences, token_lists, epoch = 5, vis_clusters = False):
        ...
        print ("Silhouette Score for the clusters is", self.get_silhouette(plot=vis_clusters))
    ...
    def get_top_words_tfidf(self, sentence_label_dict, sentences = None, sentence_token = None,  top = 200):
        '''
        ...
        if sentence_token is not None:
            for i in range(self.k):
                ...
                k_sentence = ' '.join(' '.join(word for word in sent) for sent in np.array(sentence_token)[sentence_label_dict[i]])
                #append to the list
                tfidf_sentences.append(k_sentence)
        elif sentences is not None:
            for i in range(self.k):
                # concatenate all sentences belong to cluster i into one string
                k_sentence = ' '.join(sent for sent in np.array(sentences)[sentence_label_dict[i]])
                #remove the stop words
                #tokenize
                k_sentence = word_tokenize(k_sentence)
                # remove stopwords
                k_sentence = [word for word in k_sentence if not word in stopwords.words()]
    ...
    def get_top_words_using_attention (self, sentence_label_dict,
                                       cluster_id, layer = 0, head = None, top=None):

        if self.embedding not in ['xlnet', 'bert', 'xlm', 'electra', 'albert']:
            print (" Attention vector is not available for the selected embedding model.")
            return
        ...
        cluster_attention = np.squeeze(attention_layers[:, [sentence_label_dict[cluster_id]], :, :, :])
        ...
        indexed_tokens = np.array(self.vec['embedding_tokens'])[sentence_label_dict[cluster_id]]
            ...
            if head is None:
                ...
                for attention_head in text:
                    ...
                    for token_id, token in enumerate(tokens):
                        ...
                        if tag[0][1] == 'NN' and token not in keys_to_remove and ('#' not in token):
                            ...
                            if token in attention_tokens.keys():  # if token already exists
                                attention_tokens[token] += attention_sum  # increment the value
                            else:
        ...
        attention_tokens = {k: v for k, v in sorted(attention_tokens.items(), key=lambda item: item[1], reverse=True)}
    ...
    def compress_features(self, ids):
        ...
        averaged_employee_features = pd.DataFrame(None, index=pd.Index(ids, name='Employee id'),
                                              columns=pd.Index(
                                                  ['feature_vector', 'cluster', 'feature_vector_2d',
                                                   'feature_vector_3d'],
        ...
        for employee in ids:
            ...
            averaged_employee_features['cluster'][employee] = cluster_label_per_document[
                employee_objective_indices[employee]]
    ...
    def wordcloud(self, sentences, ngram, output_directory = None):
        ...
        if self.method == 'LDA':
            ...
            for k, cluster_word_distribution in self.get_top_words_base(top=200).items():

                cluster_word_distribution_dict = dict()
                for item in cluster_word_distribution:
                    cluster_word_distribution_dict[item[0]] = item[1]

                get_wordcloud(dir=output_directory, word_distribution=cluster_word_distribution_dict, topic=k)
        else:
            ...
            for i in range(self.k):
                ...
                ngram_token_with_frequency, token_list = generate_n_gram(k_sentences, n=ngram, separator='-',
            ...
            # generate wordclouds
            for i in range(self.k):
                get_wordcloud(topic=i, word_count_dict=topic_words[i], dir=output_directory)
    ...
    def quick_stats(self, ids):
        ...
        employee_cluster_frequency = (employee_cluster_frequency/len(unique_ids))*100
    ...
    def get_coherence(self,topics = None, token_lists = None , measure='c_v', top = 100):
        ...
        def get_topic_words(token_lists, labels, k=None, top = 100):
            ...
            # get topics
            topics = list(map(lambda x: list(map(lambda x: x[0], x[:top])), word_counts))

            return topics

        if self.method == 'LDA':
            cm = CoherenceModel(model=self.basemodel, texts= self.lda_token_lists, corpus=self.corpus,
                                dictionary=self.dictionary,
                                coherence=measure)
        else:

            if token_lists is None:
                print ("Parameters missing, cannot compute coherence score for the model")
                return
            ...
            if topics is None:
                topics = get_topic_words(token_lists, self.cluster_model.labels_, top=top)
    ...
    def get_silhouette(self, plot = False):
        ...
        print("Silhouette Score after dimensionality reduction to a 3d space is ", score)
        ...
        score = silhouette_score(vec_3d, lbs)
        print("Silhouette Score after dimensionality reduction to a 2d space is ", score)
