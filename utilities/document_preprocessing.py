...
def f_noun(w_list):
    ...
    """
    return [word for (word, pos) in nltk.pos_tag(w_list) if (pos[:2] == 'NN')] #or pos[:2] == 'JJ')]
...
def preprocess_documents(file=None,
                         verbose = False, dr = 'output/preprocessed_data/', subset=None, max_sent_length= None):
    '''
    ...
    documents = documents.groupby('Id', as_index=False).agg({'Title': ' '.join,
                                                                             'FullDescription': ' '.join})
    id = documents['Id']
    docs = documents['Title'].astype(str) + ' \n ' + documents['FullDescription'].astype(str)
    ...
    if allowed_length > max_sent_length: # because a lot of embeddings don't allow to get an embedding for text with more than 500 words
        print("Allowed length according to data is {} but since it is greater than {}, reducing it to {}".format(
    ...
    id_drop = [] #place holder to save indexes for entries with non-english langauges to be dropped later.
    for idx in range(n_docs):
        ...
        if  detected_language == 'en':
            ...
            if len(text.split()) > allowed_length:
                text = text[:allowed_length]  # if the objective has more than 500 words, only take the first 500 words
    ...
    if verbose:
        print("Objectives of following employees at the following indices are being dropped: ", id_drop)
